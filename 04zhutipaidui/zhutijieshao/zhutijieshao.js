...
Page({
  ...
  // 查询是否超出免费次数
  checkRoomIsFree() {
    checkRoomIsFree(wx.getStorageSync('tokenKey')).then(res => {
      this.setData({
  ...
  toBuy() {
    if (this.data.buyType == 'pipei') {
      this.closePop()
      wx.navigateTo({
        url: '/07liebian/goumaixiwei/goumaixiwei?id=' + this.data.themeId + '&only=2',
      })
    } else if (this.data.buyType == 'haoyou') {
      this.closePop()
      wx.navigateTo({
        url: '/07liebian/goumaixiwei/goumaixiwei?id=' + this.data.themeId + '&only=1',
      })
    }
  },
  // 快速匹配
  ...
  openBuy(e) {
    ...
    if (type == 'pipei') {
      ...
      if (this.data.isFree) {
        wx.navigateTo({
          url: '/04zhutipaidui/setHouse/setHouse?id=' + this.data.themeId + '&isMatch=1' + '&inFree=' + this.data.isFree,
        })
      } else {
        // 非新人限免继续判断逻辑
      ...
      if (this.data.isFree) {
        ...
        // 非新人限免继续判断逻辑
        if (this.data.detailObj.isBao == 1 || this.data.detailObj.isFree == 1) {
          this.friendTogether();
        } else {
          this.setData({
            buyTips: true,
  ...
  friendTogether() {
    wx.navigateTo({
      url: '/04zhutipaidui/setHouse/setHouse?id=' + this.data.themeId + '&isJieBan=1' + '&inFree=' + this.data.isFree
    })
    // createBaoRoom({
